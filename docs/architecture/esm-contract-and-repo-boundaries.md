...
| Merge semantics for concurrent observations | Deferred to the evidence ledger's versioned combination policies; see `evidence-ledger-design.adoc`. Moot under delay-lockstep, where total order comes from the envelope; not moot under the relay with several independent observers. |
